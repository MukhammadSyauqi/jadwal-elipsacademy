...
<body class="bg-[#F8F9FA] font-sans text-ink-body antialiased min-h-screen flex flex-col justify-between">
    ...
    <header class="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-[#EDEDF0]">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            <!-- Brand & Lokasi Cabang -->
            <a href="{{ auth()->user()?->role === 'superadmin' ? route('superadmin.dashboard') : route('admin.dashboard', array_filter(['cabang_id' => $selectedCabang?->id, 'tanggal' => $selectedDate])) }}" class="flex items-center gap-3 group">
                ...
                <span class="text-[10px] text-secondary font-semibold uppercase tracking-wider bg-orange-50 border border-orange-200/60 px-2 py-0.5 rounded-full hidden sm:inline">CABANG {{ strtoupper($selectedCabang->nama_cabang ?? 'BUDURAN') }}</span>
            </a>
    ...
    <main class="flex-1 max-w-6xl mx-auto w-full px-4 py-8 sm:py-10 flex flex-col items-center">
        <div class="w-full max-w-[700px] flex flex-col gap-6">
            ...
            <div>
                <a href="{{ request('redirect_to', route('admin.dashboard', array_filter(['cabang_id' => $selectedCabang?->id, 'tanggal' => $selectedDate]))) }}" 
                            ...
                            <div class="relative">
                                <select name="program_id" 
                                    ...
                                    @foreach($programs as $prog)
                                        <option value="{{ $prog->id }}" data-kode="{{ $prog->kode_inisial }}" {{ old('program_id') == $prog->id ? 'selected' : '' }}>
